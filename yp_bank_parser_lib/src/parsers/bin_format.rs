...
impl YPBankBinParser {
    ...
    fn parse_record_body(body: &[u8]) -> Result<YPBankRecord, ParserError> {
        ...
        let tx_id = u64::from_be_bytes(body[offset..offset + 8].try_into().unwrap());
        offset += 8;
        ...
        let from_user_id = u64::from_be_bytes(body[offset..offset + 8].try_into().unwrap());
        offset += 8;

        // TO_USER_ID: 8 bytes, u64 big-endian
        let to_user_id = u64::from_be_bytes(body[offset..offset + 8].try_into().unwrap());
        offset += 8;

        // AMOUNT: 8 bytes, i64 big-endian
        let amount = i64::from_be_bytes(body[offset..offset + 8].try_into().unwrap());
        offset += 8;

        // TIMESTAMP: 8 bytes, u64 big-endian
        let timestamp = u64::from_be_bytes(body[offset..offset + 8].try_into().unwrap());
        offset += 8;
        ...
        let desc_len = u32::from_be_bytes(body[offset..offset + 4].try_into().unwrap());
        offset += 4;
